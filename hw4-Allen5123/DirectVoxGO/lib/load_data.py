...
from .load_blender import load_blender_data, load_blender_data_inference
...
def load_data_inference(args, testjson):

    K, depths = None, None
    near_clip = None

    if args.dataset_type == 'blender':
        poses, fnames, render_poses, hwf, i_split = load_blender_data_inference(testjson, args.half_res, args.testskip)
        i_test = i_split[0]
    ...
    HW = np.array([[800 for i in range(2)] for j in range(poses.shape[0])])
